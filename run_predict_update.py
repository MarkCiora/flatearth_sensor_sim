...
for t in map(lambda x: x * globals.dt, range(int(sim_duration/globals.dt))):
    ...
    FIG = np.empty((S,T), dtype=floattype)
    for i in range(S):
        for j in range(T):
            slew_times[i,j] = sensors[i].angle_between(targets[j].x[0:3]) / sensors[i].slew_rate
            FIG[i,j] = np.trace(targets[j].P) - np.trace(targets[j].predict_update_FI(sensors[i].p))
    slew_times = (slew_times + 0.1) ** m_exponent
    table = slew_times * FIG
